...
class Categorization(assetManager: AssetManager,modelPath: String, labelPath: String, inputSize: Int){
    ...
    fun recognizeImage (bitmap: Bitmap): List<com.example.picleaf.Categorization.Categorization>{
    ...
    private fun getSortedResult(result: Array<FloatArray>): List<com.example.picleaf.Categorization.Categorization> {
        val pq = PriorityQueue(
            GREAT_OUTCOME_MXX,
            kotlin.Comparator<com.example.picleaf.Categorization.Categorization> {
        ...
        for (i in ROW_LINE.indices){
            ...
            if (confidence >= POINT_THRHLDD){
                pq.add(com.example.picleaf.Categorization.Categorization(
                    "" +i,
                    if (ROW_LINE.size>i) ROW_LINE[i] else "Unknown", confidence
                ))
            }
        }
        val recognitions = ArrayList<com.example.picleaf.Categorization.Categorization>()
